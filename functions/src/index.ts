...
export const coachReply = onCall<CoachReplyData>(async request => {
  ...
  const parentLines = parentMsgs.matches
    ...
    [
      {
        role: 'system',
        content:
          'You are a Gottman-trained relationship coach helping a couple improve their relationship. You are analyzing their conversation and providing personalized guidance based on Gottman Method principles.',
      },
      {
        role: 'assistant',
        content: `Context of our chat so far:\n${coachLines}`,
      },
      {
        role: 'user',
        content:
    ...
    [
      {
        role: 'system',
        content:
          "You are a Gottman-trained relationship coach analyzing a couple's conversation. Provide insights based on Gottman Method research and principles.",
      },
      {
        role: 'user',
        content:
...
export const coachHorsemen = onCall<CoachHorsemenData>(async request => {
  ...
  const horsemanPercent =
    totalMessages > 0
      ? ((horsemanTotal / totalMessages) * 100).toFixed(1)
      : '0';
