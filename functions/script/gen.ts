...
async function findConversation(participants: string[]): Promise<string> {
  ...
  for (const [convId, conv] of Object.entries(conversations)) {
    ...
    if (convData.participants) {
      ...
      if (JSON.stringify(sortedExisting) === JSON.stringify(sortedParticipants)) {
  ...
  throw new Error(`Conversation not found for participants: ${participants.join(', ')}`);
}
...
async function insertMessages(
  ...
  updates[`conversations/${conversationId}/lastMessageAt`] = now - (lastMsg.minutesAgo * 60 * 1000);
...
- Emma: ${emmaId}\n`);
    ...
    const updateMessageSenderIds = (messages: MessageData[], mapping: Record<string, string>): MessageData[] => {
    ...
    const updatedHealthyMessages = updateMessageSenderIds(healthyPlatonicMessages, userIdMapping);
    const updatedTroubledMessages = updateMessageSenderIds(troubledPlatonicMessages, userIdMapping);
    const updatedGroupMessages = updateMessageSenderIds(groupMessages, userIdMapping);
