...
import {
  signupWithAutoProfile,
  loginWithCredentials,
} from './helpers/auth-simple';
...
test.describe('Comprehensive Multi-User Functionality Tests', () => {
  ...
  test('03. CRITICAL: Alice sends friend request to Bob', async ({ page }) => {
    ...
    console.log(
      `Alice (${aliceData.username}) searching for Bob (${bobData.username})`
    );
    ...
    const requestSentIndicator = page
  ...
  test("04. CRITICAL: Bob accepts Alice's friend request", async ({ page }) => {
    ...
    console.log(
      `Bob (${bobData.username}) accepting request from Alice (${aliceData.username})`
    );
    ...
    await page.screenshot({
      path: 'test-results/04-friend-requests-screen.png',
    });

    // Alice's request MUST be visible
    const aliceRequest = page
      .getByText(aliceData.username, { exact: true })
      .first();
    ...
    console.log(
      '=== STEP 4 COMPLETE: Friend request successfully accepted ==='
    );
  });

  // Step 5: Verify both users see each other as friends
  test('05. CRITICAL: Both users must see each other as friends', async ({
    page,
  }) => {
  ...
  test('06. CRITICAL: Alice starts chat with Bob', async ({ page }) => {
    ...
    await page.screenshot({
      path: 'test-results/06-chat-with-message-sent.png',
    });
  ...
  test("07. CRITICAL: Bob receives Alice's message", async ({ page }) => {
    ...
    await page.screenshot({
      path: 'test-results/07-bob-opens-chat-with-alice.png',
    });
    ...
    await page.screenshot({
      path: 'test-results/07-alice-message-visible-to-bob.png',
    });
  ...
  test('09. CRITICAL: Complete conversation verification', async ({ page }) => {
    ...
    const aliceMessageVisible = page
      .getByText(aliceMessage, { exact: true })
      .first();
    ...
    await page.screenshot({
      path: 'test-results/09-complete-conversation.png',
    });
