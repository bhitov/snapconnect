...
test.describe('Navigation Reference Tests', () => {
  test('navigation state verification using navigationRef', async ({ page }) => {
    ...
    await page.screenshot({ path: 'test-results/navigation-ref-final-state.png' });
  });

  test('navigation state tracking with console inspection', async ({ page }) => {
    const navigationLogs: string[] = [];
    
    page.on('console', message => {
      const text = message.text();
      // Capture navigation-related logs
      if (text.includes('RootNavigator') || text.includes('navigation') || text.includes('route')) {
    ...
    const tabs = ['chats-tab-button', 'camera-tab-button', 'stories-tab-button', 'friends-tab-button'];
    ...
    for (const tabTestId of tabs) {
      ...
      await page.screenshot({ path: `test-results/navigation-${tabName}-tab.png` });
    }
