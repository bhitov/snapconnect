...
test.describe('Friends and Chat Comprehensive Flows', () => {
  test('friends management: view friends, handle requests, search users', async ({
    page,
  }) => {
    page.on('console', message => {
      console.log(
        `Friends Test: ${message.type().toUpperCase()} ${message.text()}`
      );
    });
    ...
    await page.screenshot({
      path: 'test-results/friends-management-main-screen.png',
    });
    ...
    const friendRequestsButton = page
      .getByText('Friend Requests')
      .or(page.getByText('Requests'))
      .or(page.getByTestId('friend-requests-button'));
    ...
    try {
      if (await friendRequestsButton.isVisible({ timeout: 2000 })) {
        ...
        await page.screenshot({
          path: 'test-results/friends-management-requests.png',
        });

        // Look for pending requests
        const pendingRequests = page
          .getByText('Accept')
          .or(page.getByText('Decline'))
          .or(page.getByRole('button', { name: /accept|decline/i }));
        ...
        if (requestCount > 0) {
          ...
          try {
            ...
            await page.screenshot({
              path: 'test-results/friends-management-request-accepted.png',
            });
          } catch {
    ...
    const addFriendsButton = page
      .getByText('Add Friends')
      .or(page.getByText('Add'))
      .or(page.getByTestId('add-friends-button'))
    ...
    try {
      if (await addFriendsButton.isVisible({ timeout: 2000 })) {
        ...
        await page.screenshot({
          path: 'test-results/friends-management-add-friends.png',
        });

        // Look for search functionality
        const searchInput = page
          .getByPlaceholder('Search')
          .or(page.getByPlaceholder('Username or email'))
        ...
        try {
          if (await searchInput.isVisible({ timeout: 2000 })) {
            ...
            await page.screenshot({
              path: 'test-results/friends-management-search-results.png',
            });

            // Look for add buttons in search results
            const addButtons = page.getByRole('button', {
              name: /add|follow|request/i,
            });
            const addButtonCount = await addButtons.count();
            console.log(
              `Found ${addButtonCount} add buttons in search results`
            );
            ...
            if (addButtonCount > 0) {
              try {
                ...
                await page.waitForTimeout(500);
                await page.screenshot({
                  path: 'test-results/friends-management-request-sent.png',
                });
              } catch {
    ...
    await page.getByTestId('friends-tab-button').click();
    await page.screenshot({
      path: 'test-results/friends-management-final-state.png',
    });
  });

  test('chat functionality: view conversations, send messages, conversation flow', async ({
    page,
  }) => {
    page.on('console', message => {
      console.log(
        `Chat Test: ${message.type().toUpperCase()} ${message.text()}`
      );
    });
    ...
    await page.screenshot({
      path: 'test-results/chat-functionality-main-screen.png',
    });
    ...
    if (chatCount > 0) {
      ...
      try {
        ...
        await page.screenshot({
          path: 'test-results/chat-functionality-conversation-opened.png',
        });
        ...
        try {
          if (await messageInput.isVisible({ timeout: 3000 })) {
            ...
            try {
              ...
              await page.waitForTimeout(1000);
              await page.screenshot({
                path: 'test-results/chat-functionality-message-sent.png',
              });
            ...
            try {
              ...
              await page.waitForTimeout(500);
              await page.screenshot({
                path: 'test-results/chat-functionality-scrolled-messages.png',
              });
            } catch {
      ...
      const newChatButton = page
        .getByText('New Chat')
        .or(page.getByText('Compose'))
        .or(page.getByTestId('new-chat-button'))
        .or(page.getByRole('button', { name: /new|compose|\+/i }));
        ...
        if (await newChatButton.isVisible({ timeout: 2000 })) {
          ...
          try {
            if (await recipientInput.isVisible({ timeout: 2000 })) {
              ...
              await page.waitForTimeout(1000);
              await page.screenshot({
                path: 'test-results/chat-functionality-recipient-selected.png',
              });
            }
          } catch {
    ...
    await page.getByTestId('chats-tab-button').click();
    await page.screenshot({
      path: 'test-results/chat-functionality-final-state.png',
    });
  });

  test('integrated social flow: add friend, start chat, send messages', async ({
    page,
  }) => {
    page.on('console', message => {
      console.log(
        `Integrated Test: ${message.type().toUpperCase()} ${message.text()}`
      );
    });
    ...
    await page.getByTestId('friends-tab-button').click();
    await page.screenshot({
      path: 'test-results/integrated-1-friends-start.png',
    });
    ...
    for (const action of friendsActions) {
      ...
      try {
        if (await element.isVisible({ timeout: 1000 })) {
          ...
          await page.screenshot({
            path: `test-results/integrated-friends-${action.toLowerCase().replace(' ', '-')}.png`,
          });
    ...
    await page.screenshot({
      path: 'test-results/integrated-2-chats-start.png',
    });

    // Look for existing conversations or start new ones
    const chatsList = page.locator(
      '[data-testid*="chat"], [data-testid*="conversation"]'
    );
    ...
    if (existingChats > 0) {
      ...
      try {
        ...
        await page.screenshot({
          path: 'test-results/integrated-3-conversation-opened.png',
        });
        ...
        for (let i = 0; i < testMessages.length; i++) {
          try {
            if (await messageInput.isVisible({ timeout: 2000 })) {
              ...
              await page.screenshot({
                path: `test-results/integrated-message-${i + 1}.png`,
              });
            } else {
      ...
      try {
        if (await newChatButton.isVisible({ timeout: 2000 })) {
          ...
          await page.screenshot({
            path: 'test-results/integrated-3-new-chat.png',
          });
        }
      } catch {
    ...
    await page.waitForTimeout(300);
    await page.screenshot({
      path: 'test-results/integrated-4-back-to-friends.png',
    });
