...
const requiredVars = ['OPENAI_API_KEY', 'PINECONE_API_KEY', 'PINECONE_ENV', 'PINECONE_INDEX'];
const missingVars = requiredVars.filter(key => !config[key as keyof Config]);

if (missingVars.length > 0) {
  console.warn(`⚠️  Missing environment variables in datagen/.env.local: ${missingVars.join(', ')}`);
}
