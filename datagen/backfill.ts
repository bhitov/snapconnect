...
async function classify(
  ...
  console.log(
    `🏷️  [CLASSIFY] Messages:`,
    batch.map(
      (t, i) => `${i + 1}. "${t.slice(0, 50)}${t.length > 50 ? '...' : ''}"`
    )
  );
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      ...
      console.log(
        `🏷️  [CLASSIFY] Unexpected format, converting object to array`
      );
      result = Object.values(parsed) as {
        sentiment: string;
        horseman: string;
      }[][];
    }
